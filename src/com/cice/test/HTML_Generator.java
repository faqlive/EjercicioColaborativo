		...
		try (BufferedWriter outputStream = new BufferedWriter(
			...
			while ((line = plantilla.readLine()) != null) {
				if (line.contains("${Titulo}"))
					line = line.replace("${Titulo}", v.getTitulo());
				if (line.contains("${Plataforma}"))
					line = line.replace("${Plataforma}", v.getPlataforma());
				if (line.contains("${Precio}"))
					line = line.replace("${Precio}", String.valueOf(v.getPrecio()));
